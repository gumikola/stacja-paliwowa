...
QVector<int> CalculateOrder::GetDistanceVector(QVector<Common::CustomerStruct> customers,
                                               int                             CustomerID)
{
    const QString pathCachedJsons         = "stacja-paliwowa/Jsons/";
    const QString AbsolutepathCachedJsons = QDir::currentPath().split("build")[0] +
                                            pathCachedJsons + "test" + QString::number(CustomerID) +
                                            ".txt";

    QString       json;
    QFileInfo     check_file(AbsolutepathCachedJsons);
    QJsonDocument jsonResponse;
    if (check_file.exists() && check_file.isFile())
    {
        QFile file;
        file.setFileName(AbsolutepathCachedJsons);
        file.open(QIODevice::ReadOnly | QIODevice::Text);
        json = file.readAll();
        file.close();
        jsonResponse = QJsonDocument::fromJson(json.toUtf8());
    }
    else
    {
        ...
        {
            originString.append(customers[CustomerID].city + "_" + customers[CustomerID].street +
                                "_" + customers[CustomerID].propertyNumber);
        ...
        event.exec();
        jsonResponse = QJsonDocument::fromJson(response->readAll());
        QFile file(AbsolutepathCachedJsons);

        file.open(QIODevice::ReadWrite);
        qDebug() << "Otwarlem plik nowy";
...
QVector<QVector<Common::OrdersStruct>> CalculateOrder::GetOrder()
{
    ...
    QPair<QVector<CalculateOrder::CustomerDistanceID>, int> order =
        CalculateOrderFor2Or1Trucks(true);
    /*zawsze dla dwoch do testow if (order.second < 6 * 60 * 60)
    {
...
CalculateOrder::drawPosition(QVector<CalculateOrder::CustomerDistanceID> Customers)
{
    ...
    while (isOk == false)
    {
        ...
        {
            if (Customers[i].isHome && Customers[i + 1].isHome)
                isOk = false;
        }
    }
                                              ...
                                              bool                                        twoTrucks)
{
    ...
    if (twoTrucks)
    {
        ...
        while (1)
        {
            if (Order[i].isHome && i != 0)
                break;
            totalTime.first += Order[i].distancesTable[Order[i + 1].ID] + 30 * 60;
            i++;
        }

        while (i < Order.size() - 1)
        {
            totalTime.second += Order[i].distancesTable[Order[i + 1].ID] + 30 * 60;
            i++;
        }
        totalTime.second -= 30 * 60;
    ...
    totalTime.first -= 30 * 60;
    // zmiany
    if (totalTime.first > 60 * 60 * 6 || totalTime.second > 60 * 60 * 6)
    {
        totalTime.first  = 1000000;
        totalTime.second = 1000000;
    }
    // zmiany
    return totalTime;
}
...
CalculateOrder::CalculateOrderFor2Or1Trucks(bool twoTrucks)
{
    ...
    int bestDuration = INT32_MAX;

    QVector<CalculateOrder::CustomerDistanceID> bestOrder = Customers;
    QVector<CalculateOrder::CustomerDistanceID> testOrder = Customers;

    for (int i = 0; i < 100000; i++)
    {
        testOrder    = drawPosition(testOrder);
        testDuration = (CalculateTime(testOrder, twoTrucks).first +
                        CalculateTime(testOrder, twoTrucks).second);
